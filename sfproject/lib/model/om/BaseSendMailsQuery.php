 ...
 * @method SendMails findOneByResult(boolean $result) Return the first SendMails filtered by the result column
 ...
 * @method array findByResult(boolean $result) Return SendMails objects filtered by the result column
...
abstract class BaseSendMailsQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterByResult(true); // WHERE result = true
     * $query->filterByResult('yes'); // WHERE result = true
     * </code>
     *
     * @param     boolean|string $result The value to use as filter.
    ...
    {
        if (is_string($result)) {
            $result = in_array(strtolower($result), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }
